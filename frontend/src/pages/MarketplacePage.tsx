...
import { Search, Star, ShieldCheck, Unlock, Coins } from 'lucide-react';
    ...
    <div className="min-h-full bg-background p-6 md:p-10 overflow-y-auto">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-4xl font-bold tracking-tight mb-2">Template Store</h1>
            <p className="text-muted-foreground text-lg">Discover premium, ATS-optimized designs.</p>
          </div>
          ...
          {categories.map((cat) => (
            <button key={cat} onClick={() => setSelectedCategory(cat)}
              className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${
                selectedCategory === cat ? 'bg-foreground text-background' : 'bg-card border border-border text-muted-foreground hover:text-foreground'
              }`}>
              {cat}
            </button>
          ))}
            ...
            return (
              <div key={template.id}
                ...
                <div className="aspect-[1/1.2] bg-muted relative overflow-hidden p-4">
                  <div className="w-full h-full bg-card rounded-md shadow-sm border border-border/50 flex flex-col p-4">
                  ...
                  <div className="absolute top-3 left-3">
                    <div className="bg-background/90 backdrop-blur text-xs font-bold px-2 py-1 rounded-md shadow-sm flex items-center gap-1">
                      <ShieldCheck size={12} className="text-emerald-500" /> ATS {template.atsScore}
                    </div>
                  </div>
                  <div className="absolute top-3 right-3">
                    {isOwned ? (
                      ...
                      <div className="bg-background/90 backdrop-blur text-xs font-bold px-2 py-1 rounded-md shadow-sm flex items-center gap-1">
                        {isFreeForUser ? 'Free' : <span className="flex items-center gap-1 text-amber-500"><Coins size={12} />{template.cost}</span>}
                      </div>
                    )}
                  </div>
                  <div className="absolute inset-0 bg-background/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <Button variant="primary" className="shadow-xl"
                      onClick={(e) => {
                        e.stopPropagation();
                        if (isOwned) { onTemplateSelected(template.id); onNavigateToEditor(); }
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground font-medium">
                    ...
                    <span className="flex items-center gap-1"><Star size={12} className="text-amber-500 fill-amber-500" />{template.popularity}%</span>
                  </div>
                </div>
              </div>
            );
      ...
      {selectedTemplate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setSelectedTemplate(null)} />
          <div className="relative w-full max-w-lg glass-panel rounded-3xl overflow-hidden shadow-2xl border-border">
            <div className="p-8">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h2 className="text-2xl font-bold mb-2">{selectedTemplate.name}</h2>
                  <div className="flex items-center gap-3 text-sm">
                    <span className="px-2 py-1 bg-muted rounded-md font-medium">{selectedTemplate.category}</span>
                    <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-medium">
                      <ShieldCheck size={14} /> ATS {selectedTemplate.atsScore}
                    </span>
                  </div>
                </div>
                {!ownedTemplateIds.includes(selectedTemplate.id) && (
                  <div className="flex flex-col items-end">
                    <span className="text-xs text-muted-foreground uppercase tracking-wider font-bold mb-1">Cost</span>
                    <div className="flex items-center gap-1.5 text-xl font-bold text-amber-500">
                      <Coins size={20} />{selectedTemplate.cost}
                    </div>
                  </div>
                )}
              </div>
              <p className="text-muted-foreground mb-8 leading-relaxed">{selectedTemplate.description}</p>
              <div className="flex gap-3">
                <Button variant="outline" className="flex-1" onClick={() => setSelectedTemplate(null)}>Cancel</Button>
                {ownedTemplateIds.includes(selectedTemplate.id) ? (
                  <Button className="flex-1" onClick={() => { onTemplateSelected(selectedTemplate.id); onNavigateToEditor(); setSelectedTemplate(null); }}>
                    Use Template
                  </Button>
                ) : (
                  <Button className="flex-1 bg-gradient-to-r from-amber-500 to-orange-500 text-white border-0 hover:opacity-90"
                    onClick={() => handleUnlock(selectedTemplate)}>
                    <Unlock size={16} className="mr-2" />
                    {subscriptionTier === 'premium' ? 'Unlock (Free with Premium)' : `Unlock for ${selectedTemplate.cost} Points`}
                  </Button>
                )}
              </div>
            </div>
