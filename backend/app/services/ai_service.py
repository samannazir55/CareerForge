...
# No longer using local models
MODEL_ID = "microsoft/Phi-3-mini-4k-instruct"  
model = None
tokenizer = None

def get_client():
    # Priority: OpenRouter > Groq > OpenAI > None
    ...
    return OpenAI(
        ...
        base_url=base_url,
        default_headers={
            "HTTP-Referer": "https://careerforge.onrender.com", # Recommended by OpenRouter
            "X-Title": "CareerForge",                           # Recommended by OpenRouter
        }
    )
...
def chat_with_user(history: List[Dict[str, Any]], latest_message: str) -> Dict[str, Any]:
    ...
    if os.getenv("OPENROUTER_API_KEY"):
        model_name = "meta-llama/llama-3.3-70b-instruct" # Equivalent Llama 3.3 model on OpenRouter
    elif os.getenv("GROQ_API_KEY"):
...
def generate_cv_content_from_ai(request: ai_schemas.AIGenerationRequest) -> ai_schemas.AIResponse:
    ...
    try:
        ...
        if request.personal_strengths and "SUMMARIZE THIS RESUME:" in request.personal_strengths:
            ...
            raw_text = clean_pdf_text(full_text)[:4000]  
