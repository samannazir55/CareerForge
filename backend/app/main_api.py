...
def register(user: user_schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    # Return pending status — no token yet, user must verify email first
    return {
...
def verify_otp(payload: user_schemas.OTPVerify, db: Session = Depends(get_db)):
    result = user_crud.verify_otp(db, payload.email, payload.otp)
    if not result["success"]:
        raise HTTPException(status_code=400, detail=result["error"])
    db_user = user_crud.get_user_by_email(db, payload.email)
    token = security.create_jwt_token({"user_id": db_user.id, "email": db_user.email})
    return {"access_token": token, "token_type": "bearer"}

@router.post("/auth/resend-otp")
def resend_otp(payload: user_schemas.OTPResend, db: Session = Depends(get_db)):
    result = user_crud.resend_otp(db, payload.email)
    if not result["success"]:
...
# ---------------------------------------------------------
# TEMPLATE UNLOCK ENDPOINTS
# ---------------------------------------------------------
@router.post("/templates/{template_id}/unlock")
def unlock_template_endpoint(
    template_id: str,
    db: Session = Depends(get_db),
    user: dict = Depends(get_current_user)
):
    """Unlock a premium template for the current user (deducts 1 credit)."""
    db_user = user_crud.get_user(db, int(user["user_id"]))
    if not db_user:
        raise HTTPException(404, "User not found")

    # Premium subscribers get everything free
    if db_user.subscription_plan == "premium":
        user_crud.unlock_template(db, db_user.id, template_id)
        return {"success": True, "message": "Template unlocked via Premium plan."}

    # Check the template exists and is premium
    tmpl = template_crud.get_template(db, template_id)
    if not tmpl:
    ...
    # Deduct credit
    if (db_user.credits or 0) < 1:
        raise HTTPException(402, "Insufficient credits. Please upgrade your plan.")

    db_user.credits = (db_user.credits or 0) - 1
    db.commit()
    user_crud.unlock_template(db, db_user.id, template_id)
    return {"success": True, "message": "Template unlocked!", "credits_remaining": db_user.credits}

@router.get("/templates/unlocked")
def get_unlocked(
    db: Session = Depends(get_db),
    user: dict = Depends(get_current_user)
):
    """Return list of template IDs unlocked by the current user."""
    unlocked = user_crud.get_unlocked_templates(db, int(user["user_id"]))
...
def export_endpoint(cv_id: int, type: str, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    ...
    # ── Premium template download guard ───────────────────────────────────────
    db_user   = user_crud.get_user(db, int(user["user_id"]))
    tmpl_id   = str(db_cv.template_id)
    tmpl_obj  = template_crud.get_template(db, tmpl_id)
    is_premium_tmpl = tmpl_obj and tmpl_obj.is_premium

    if is_premium_tmpl:
        has_access = (
            db_user and (
                db_user.subscription_plan == "premium"
                or tmpl_id in (db_user.unlocked_templates or "").split(",")
            )
        )
        if not has_access:
            raise HTTPException(
                status_code=402,
                detail="PREMIUM_TEMPLATE_LOCKED"
            )
...
def setup_production_db(db: Session = Depends(get_db)):
    ...
    # 1. MIGRATE SCHEMA
    migrations = [
        ("credits",             "ALTER TABLE users ADD COLUMN credits INTEGER DEFAULT 3"),
        ...
        ("otp_resend_count",    "ALTER TABLE users ADD COLUMN otp_resend_count INTEGER DEFAULT 0"),
        ("otp_resend_at",       "ALTER TABLE users ADD COLUMN otp_resend_at DATETIME"),
        ("unlocked_templates",  "ALTER TABLE users ADD COLUMN unlocked_templates VARCHAR DEFAULT ''"),
    ]
    for col_name, sql in migrations:
        try:
            # SQLite-compatible column existence check
            ...
            if col_name not in cols:
                ...
                log.append(f"ℹ️  Column exists: {col_name}")
        except Exception as e:
            log.append(f"⚠️  {col_name}: {e}")
